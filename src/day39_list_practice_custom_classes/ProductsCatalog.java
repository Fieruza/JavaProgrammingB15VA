...
​
        ...
        for(int n = 0; n < items.size(); n++){
            if (items.get(n).toLowerCase().contains("iphone")){
                System.out.println(items.get(n) + " Found at index " + n);
                items.remove(n);
                prices.remove(n);
                monthlyPayments.remove(n);
                n--; //take one index back.
                //System.out.println("items = " + items);
            }
        }

        //RemoveIf uses lambda expression
        //items.removeIf(p -> p.toLowerCase().contains("iphone"));
...
​
        ...
        int minPriceIndex = 0;
        for (int i = 0; i <prices.size() ; i++) {
            if(prices.get(i) < prices.get(minPriceIndex)){
                minPriceIndex = i;
            }
        }
